...
public class ExpenseServiceImpl implements ExpenseService {
	...
	public String uploadFile(MultipartFile file) {
		...
		try {
			awsS3Client.putObject("javaproject222", key, file.getInputStream(), metaData);
		} catch (IOException e) {
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
					"An exception occured while uploading the file");
		}

		awsS3Client.setObjectAcl("javaproject222", key, CannedAccessControlList.PublicRead);

		return awsS3Client.getResourceUrl("javaproject222", key);
	}
